...
class Cell extends Component {
  ...
  render() {
    ...
    let cellText =
      this.props.data.mineCount === 0 ? "" : this.props.data.mineCount;
    let content = this.props.data.isMine ? "" : cellText;
    if (this.props.data.status === "show") {
      className +=
        " ShowCell " + numberWords[this.props.data.mineCount] + "Cell";
    }
    if (this.props.data.status === "hidden") {
    ...
    if (this.props.gameStatus === "loss") {
      ...
      if (this.props.data.status === "loss") {
        ...
        this.props.data.status === "flagged" &&
    ...
    return (
      <div
        ...
        onContextMenu={this.handleClickDown}
      >
        {content}
      </div>
    );
